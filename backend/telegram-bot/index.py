...
def ask_ai_assistant(question: str, is_registered: bool = False) -> str:
    """AI помощник на основе YandexGPT с веб-поиском"""
    try:
        system_prompt = f"""Ты AI-помощник курьерского сервиса Stuey.Go для курьеров Яндекс.Еды.

Твоя задача — отвечать на вопросы курьеров максимально понятно и полезно.

<b>Информация о Stuey.Go:</b>
- Платформа для рекрутинга курьеров Яндекс.Еды  
- Сайт: stuey-go.ru
- Регистрация занимает 10 минут, первые заказы через 2 часа
- Заработок от доставок: 40,000-165,000₽/месяц (зависит от города и транспорта)

<b>Реферальная программа:</b>
- 12,000₽ за каждого приглашённого друга
- Условие: друг должен выполнить 150 заказов
- БЕЗ ограничений по количеству рефералов
- Примеры: 5 друзей = 60,000₽, 10 = 120,000₽, 20 = 240,000₽

<b>Самобонус:</b>
- 5,000₽ за собственные 150 выполненных заказов
...
<b>Условия работы:</b>
- Гибкий график (работай когда хочешь)
...
Отвечай кратко, по делу, используй эмодзи для наглядности. Если вопрос не про курьерку - всё равно помоги!"""
...
• "Как работать в Яндекс.Еде?"""
                        ...
                        answer = ask_ai_assistant(text, is_registered=True)
...
• "Как работать в Яндекс.Еде?"""
                        ...
                        # Любой другой текст = вопрос к AI
                        send_telegram_message(chat_id, "🤖 Думаю...")
                        answer = ask_ai_assistant(text, is_registered=False)
                        send_telegram_message(chat_id, f"🤖 <b>AI Помощник:</b>\n\n{answer}")
