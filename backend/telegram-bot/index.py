...
- 📦 Выполнено заказов: {total_orders} {'✅ Самобонус получен!' if total_orders >= 30 else f'🔥 До самобонуса 5000₽ осталось: {orders_to_bonus} заказов!'}
- 👥 Всего рефералов: {referrals}
- ⭐ Активных рефералов (30+ заказов): {active_referrals} (заработано: {active_referrals * 5000}₽)
- 🔥 Близко к активации (20-29 заказов): {close_to_active} человек (потенциал: {close_to_active * 5000}₽)
...
1. **Стартовый самобонус** — 5000₽ за первые 30 заказов (разовый)
2. **Реферальный бонус** — 5000₽ за КАЖДОГО реферала, выполнившего 30 заказов (без ограничений!)
...
→ "👥 У тебя {referrals} рефералов! Напиши им, поддержи — когда они сделают 30 заказов, ты получишь {referrals * 5000}₽!"

Если близко к активации {close_to_active} > 0:
→ "🔥 У тебя {close_to_active} {'реферал' if close_to_active == 1 else 'реферала'} почти у цели (20-29 заказов)! Напиши им, поддержи — скоро получишь +{close_to_active * 5000}₽!"
    ...
    try:
        ...
        cursor.execute("""
            SELECT 
                ...
                COUNT(*) FILTER (WHERE total_orders >= 20 AND total_orders < 30) as close_to_active,
...
def verify_and_link_code(chat_id: int, telegram_id: int, username: Optional[str], code: str):
    ...
    try:
        ...
        text = (
            ...
            f"<b>🎁 Твои бонусы:</b>\n"
            f"💰 Сделай 30 заказов = получи 5000₽\n"
