...
def detect_search_intent(question: str, courier_city: str = None) -> Optional[Dict[str, Any]]:
    """Определить нужен ли поиск в интернете"""
    ...
    detected_city = courier_city
    for key, city in cities_map.items():
        if key in question_lower:
            detected_city = city
            break
    
    # Паттерны вопросов требующих поиска
    
    # 1. Аренда транспорта
    if any(word in question_lower for word in ['аренд', 'снять', 'взять напрокат', 'где взять']):
        if any(word in question_lower for word in ['велосипед', 'велик', 'вело']):
            if not detected_city:
                return {'type': 'ask_city', 'topic': 'велосипед'}
            return {
                'type': 'search',
                'query': f"аренда велосипеда для курьеров {detected_city} Яндекс 2024",
                'topic': 'rent_bike',
                'city': detected_city
            }
        
        if any(word in question_lower for word in ['самокат', 'скутер', 'электросамокат']):
            if not detected_city:
                return {'type': 'ask_city', 'topic': 'самокат'}
            return {
                'type': 'search',
                'query': f"аренда самоката для курьеров {detected_city} Яндекс 2024",
                'topic': 'rent_scooter',
                'city': detected_city
            }
    
    # 2. Как устроиться / начать работать
    if any(phrase in question_lower for phrase in ['как устроиться', 'как начать', 'стать курьером', 'устройство на работу']):
        return {
            'type': 'search',
            'query': f"как устроиться курьером Яндекс {detected_city or 'Россия'} 2024 требования",
            'topic': 'how_to_start',
            'city': detected_city
        }
    
    # 3. Конкретные адреса / точки
    if any(phrase in question_lower for phrase in ['где находится', 'адрес', 'точка выдачи', 'офис']):
        if not detected_city:
            return {'type': 'ask_city', 'topic': 'адрес'}
        return {
            'type': 'search',
            'query': f"офис Яндекс для курьеров {detected_city} адрес 2024",
            'topic': 'office_address',
            'city': detected_city
        }
    
    # 4. Требования / документы
    if any(word in question_lower for word in ['требования', 'документы', 'что нужно', 'нужен ли']):
        return {
            'type': 'search',
            'query': f"требования к курьерам Яндекс {detected_city or 'Россия'} документы 2024",
            'topic': 'requirements',
            'city': detected_city
        }
...
def ask_yandex_gpt(question: str, context: Dict[str, Any], cursor) -> str:
    """Спросить YandexGPT о чём угодно с умным поиском в интернете"""
    ...
    courier_city = context.get('city', None)
    search_intent = detect_search_intent(question, courier_city)
    
    web_info = ""
    
    if search_intent:
        if search_intent['type'] == 'ask_city':
            topic = search_intent['topic']
            return f"""🏙️ В каком городе тебе нужна информация про {topic}?
            
Напиши название города (например: Москва, Санкт-Петербург, Екатеринбург) и я найду актуальную информацию!"""
        
        elif search_intent['type'] == 'search':
            web_info = search_web(search_intent['query'])
            print(f'📡 Search results for topic {search_intent["topic"]}: {web_info[:150]}...')
    
    # Добавить информацию из поиска в контекст если есть
    web_context = ""
    if web_info:
        web_context = f"""
...
⚠️ ВАЖНО: Эта информация из интернета ПРИОРИТЕТНА! Используй её для ответа на вопрос курьера."""
    ...
    if is_new_user:
        # Промпт для новичков без привязки аккаунта
        system_prompt = f"""Ты — дружелюбный AI-ассистент Stuey.Go. Этот человек ещё НЕ зарегистрирован на сайте. Твоя ГЛАВНАЯ задача — замотивировать зарегистрироваться на https://stuey-go.ru!

💼 ТВОЯ РОЛЬ — ПРОДАВАТЬ РЕГИСТРАЦИЮ:
1. **Отвечай на любые вопросы** — но ВСЕГДА приводи к регистрации на сайте
2. **Рассказывай про возможности Stuey.Go** — реферальная программа, игры, статистика
3. **Мотивируй конкретными цифрами** — сколько можно заработать дополнительно
4. **Делай регистрацию желанной** — это не просто сайт, это инструмент для заработка!

🌐 ЧТО ДАЁТ РЕГИСТРАЦИЯ НА STUEY.GO:

🎁 **РЕФЕРАЛЬНАЯ ПРОГРАММА - ГЛАВНАЯ ФИШКА!**
- 5,000₽ за КАЖДОГО друга, который станет активным курьером (50 заказов)
- БЕЗ ОГРАНИЧЕНИЙ! 10 друзей = 50,000₽, 20 друзей = 100,000₽, 50 друзей = 250,000₽!
- Отслеживай прогресс каждого реферала в реальном времени
- Видишь сколько заказов сделал каждый друг и когда получишь деньги
- Персональная реферальная ссылка — отправляй в WhatsApp/Telegram

💰 **ДОПОЛНИТЕЛЬНЫЙ ЗАРАБОТОК:**
- Основная работа курьером: 40,000-120,000₽/месяц
- + Самобонус за старт: 5,000₽ (первые 50 заказов)
- + Рефералы: НЕОГРАНИЧЕННО! Даже 3-5 активных друзей = +25,000₽
- = ИТОГО можешь зарабатывать в 1.5-2 раза больше!

📊 **ЛИЧНЫЙ КАБИНЕТ:**
- Полная статистика заработка в реальном времени
- Видишь сколько заработал сегодня/неделю/месяц
- Прогресс до самобонуса 5,000₽
- Детализация по рефералам — кто сколько принёс денег
- История выплат

🎮 **ИГРЫ С БОНУСАМИ:**
- Мини-игры прямо в боте (доступны после регистрации)
- Зарабатывай дополнительные бонусы играя
- Соревнуйся с другими курьерами в рейтинге
- Развлечение + деньги!

💸 **УДОБНЫЕ ВЫПЛАТЫ:**
- Подавай заявки на вывод через сайт или бот
- От 5,000₽ через СБП на любую карту
- Обработка 1-3 рабочих дня
- Вся история выплат в одном месте

📱 **ЭТОТ TELEGRAM-БОТ:**
- После регистрации получишь доступ ко всем функциям бота
- Статистика прямо в Telegram
- Уведомления о рефералах
- AI-ассистент с персональными советами
- Игры и развлечения

⚠️ КРИТИЧЕСКИ ВАЖНО:
- ФОКУС только на регистрации на НАШЕМ сайте stuey-go.ru
- Если спрашивают "Как устроиться?" → "Зарегистрируйся на stuey-go.ru! После регистрации получишь полную инструкцию, включая информацию про приложение Яндекс Про"
- Если спрашивают "Сколько зарабатывают?" → "40-120k/месяц + реферальная программа на сайте (до 250k дополнительно!)"
- Про Яндекс Про упоминай ТОЛЬКО после того, как человек зарегистрируется на нашем сайте
- ВСЕГДА заканчивай ответ призывом зарегистрироваться

🎯 ПРИМЕРЫ ПРАВИЛЬНЫХ ОТВЕТОВ:

Вопрос: "Сколько можно заработать?"
Ответ: "💰 Курьеры зарабатывают 40,000-120,000₽/месяц! Но это не всё — на нашем сайте stuey-go.ru есть реферальная программа: от 18,000₽ за каждого друга! Даже 5 друзей = +90,000₽+! Зарегистрируйся и получи персональную ссылку: https://stuey-go.ru 🚀"

Вопрос: "Как устроиться курьером?"
Ответ: "🎯 Всего 3 шага:
1️⃣ Зарегистрируйся на stuey-go.ru (2 минуты)
2️⃣ Получи инструкцию и реферальную ссылку (от 18,000₽ за друга!)
3️⃣ Следуй пошаговой инструкции — мы расскажем про документы и приложение Яндекс Про
Начни здесь: https://stuey-go.ru 🔥"

Вопрос: "Что это за бот?"
Ответ: "👋 Я помощник Stuey.Go! После регистрации на сайте получишь доступ к реферальной программе (от 18,000₽ за друга!), статистике заработка, играм и полному функционалу бота! Регистрируйся: https://stuey-go.ru 💰"

Вопрос: "Расскажи подробнее"
Ответ: "🌟 Stuey.Go — это платформа для курьеров с реферальной программой! Главная фишка: приглашай друзей и получай от 18,000₽ за каждого (без лимитов!). Плюс статистика, игры, удобные выплаты. Регистрация 2 минуты → https://stuey-go.ru 🚀"

📍 ВСЕГДА ДАВАЙ ССЫЛКУ: https://stuey-go.ru

Будь энергичным и мотивирующим! Твоя цель — чтобы человек ЗАХОТЕЛ зарегистрироваться ПРЯМО СЕЙЧАС! 🔥"""
    else:
        # Промпт для существующих курьеров
...
2. **Реферальный бонус** — от 18,000₽ за КАЖДОГО реферала, выполнившего {self_bonus_orders} заказов (точная сумма зависит от города друга!)
3. **Выплаты** — от {min_withdrawal}₽ через СБП, обработка {bot_settings.get('withdrawal_processing_days', '1-3 дня')}
4. **Потенциал заработка** — НЕОГРАНИЧЕННЫЙ! 5 рефералов = от 90,000₽+, 10 рефералов = от 180,000₽+!
...
→ "👥 У тебя {referrals} рефералов! Напиши им, поддержи — когда они сделают {self_bonus_orders} заказов, можешь получить от 18,000₽ за каждого активного!"

Если близко к активации {close_to_active} > 0:
→ "🔥 У тебя {close_to_active} {'реферал' if close_to_active == 1 else 'реферала'} почти у цели (40-49 заказов)! Напиши им, поддержи — скоро получишь от 18,000₽+ за каждого!"
...
→ "⭐ Красавчик! {active_referrals} активных рефералов уже приносят тебе доход! Продолжай делиться ссылкой — каждый новый друг это от 18,000₽+!"
...
Ответ: "💡 Три способа: 1️⃣ Работай в часы пик (12-14, 18-20) — заказов больше на 50%! 2️⃣ Активность в выходные — заработок выше на 30-50%! 3️⃣ Рефералы = пассивный доход! Каждый активный реферал приносит от 18,000₽+ БЕЗ ОГРАНИЧЕНИЙ! 🚀"

🔍 ПОМОЩЬ ПО ПРАКТИЧЕСКИМ ВОПРОСАМ:
- Если курьер спрашивает про аренду велосипеда/самоката — расскажи про Яндекс Про (аренда за 1₽) и укажи на актуальную информацию ниже
- Если спрашивают "Где арендовать?" без города — ОБЯЗАТЕЛЬНО спроси в каком городе
- Используй информацию из интернета (если есть) для точных ответов про адреса, цены, условия
- Всегда давай конкретные советы: "Открой приложение Яндекс Про → Профиль → Аренда транспорта"

💡 ЯНДЕКС ПРО - АРЕНДА ТРАНСПОРТА:
В приложении Яндекс Про есть раздел аренды велосипедов и самокатов для курьеров по специальным условиям (обычно от 1₽). Это самый выгодный вариант!
Чтобы найти: откройте приложение → нажмите "Профиль" → найдите "Аренда транспорта" или "Велосипеды"
{web_context}
"""
...
def verify_and_link_code(chat_id: int, telegram_id: int, username: Optional[str], code: str):
    ...
    try:
        ...
        text = (
            ...
            f"<b>🎁 Твои бонусы:</b>\n"
            f"💰 Сделай 50 заказов = получи 5,000₽\n"
            f"👥 Приведи друга = от 18,000₽ за каждого активного!\n\n"
    ...
    if courier_id and len(parts) < 2:
        ...
        try:
            ...
            if referrals > 0 and active_referrals == 0:
                text += f"💡 <b>Совет:</b> У тебя {referrals} {'реферал' if referrals == 1 else 'рефералов'}! Напиши им, поддержи — можешь заработать от 18,000₽ за каждого активного!\n\n"
...
def handle_bonus_command(chat_id: int, telegram_id: int):
    ...
    try:
        ...
        if is_completed:
            text = (
                f"🎉 <b>Самобонус {int(bonus_earned)}₽!</b>\n\n"
                f"✅ Поздравляем!\n"
...
def handle_faq_command(chat_id: int, telegram_id: int, faq_type: str = 'menu'):
    ...
    if faq_type == 'earnings':
        ...
        text = (
            ...
            "<b>1️⃣ Подай заявку (5 минут)</b>\n"
            "• Нажми кнопку «Подать заявку» ниже\n"
            "• Заполни анкету (ФИО, телефон, город)\n"
            "• Загрузи фото документов (паспорт)\n\n"
            "<b>2️⃣ Проверка (10-30 минут)</b>\n"
            "• Документы проверят автоматически\n"
            "• Получишь SMS с инструкцией\n"
            "• Скачай приложение Яндекс Про\n\n"
            "<b>3️⃣ Первый заказ (в тот же день!)</b>\n"
            "• Включи статус «Готов к заказам»\n"
            "• Жди первый заказ (5-10 минут)\n"
        ...
        keyboard = {
            'inline_keyboard': [
                [{'text': '📝 Подать заявку на курьера', 'url': 'https://reg.eda.yandex.ru/?advertisement_campaign=forms_for_agents&user_invite_code=f123426cfad648a1afadad700e3a6b6b&utm_content=blank'}],
                [{'text': '⬅️ Назад в FAQ', 'callback_data': 'faq_menu'}]
            ]
        }
        ...
        try:
            ...
            text = (
                ...
                f"<b>🎁 Зарабатывай с каждого друга!</b>\n\n"
                f"<b>Как это работает:</b>\n"
                f"1️⃣ Пригласи друга по ссылке\n"
                f"2️⃣ Он регистрируется курьером\n"
                f"3️⃣ Делает {referral_orders} заказов\n"
                f"4️⃣ Ты получаешь от 18,000₽!\n\n"
                f"💡 <b>Точная сумма зависит от города друга</b>\n"
                f"и обновляется еженедельно.\n"
                f"Смотри актуальный размер бонуса в личном кабинете!\n\n"
                f"<b>🎯 Кого приглашать:</b>\n"
                f"• Студентов (гибкий график!)\n"
                f"• Тех, кто ищет подработку\n"
                f"• Друзей с велосипедом\n"
...
def handle_text_message(chat_id: int, telegram_id: int, text: str):
    ...
    # Если аккаунт не привязан — всё равно отвечаем через AI для рекрутинга
    if not courier_id:
        # Создаём базовый контекст для новичка
        context = {
            'balance': 0,
            'total_orders': 0,
            'referrals': 0,
            ...
            'need_motivation': 0,
            'never_worked': 0,
            'is_new_user': True  # Флаг что это новичок
        }
    else:
        update_last_interaction(telegram_id)
        # Получить полный контекст курьера
        context = get_courier_context(courier_id)
