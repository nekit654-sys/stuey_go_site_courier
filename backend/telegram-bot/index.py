...
def detect_city_context(question: str, courier_city: str = None) -> Optional[str]:
    """Определить город из вопроса или профиля курьера"""
...
def ask_yandex_gpt(question: str, context: Dict[str, Any], cursor) -> str:
    """Спросить YandexGPT о чём угодно с практическими знаниями"""
    if not YANDEX_GPT_API_KEY or not YANDEX_FOLDER_ID:
        return "🤖 AI-ассистент временно недоступен. Используйте команды из меню."
    
    # Определить город из контекста
    courier_city = context.get('city', None)
    detected_city = detect_city_context(question, courier_city)
    
    # Добавим контекст города если есть
    city_context = ""
    if detected_city:
        city_context = f"\n\n🏙️ ГОРОД КУРЬЕРА: {detected_city} (учитывай это в ответах про адреса, аренду, офисы)"
...
🌟 **STUEY.GO В ЦИФРАХ:**
- 👥 2,847 курьеров уже зарегистрировалось
- ⭐️ 4.8/5 средняя оценка платформы
- 💵 127,000₽ выплачено рефералам в декабре
...
- НИКОГДА не отправляй на сторонние сайты или приложения!
- Если спрашивают "Как устроиться?" → "Зарегистрируйся на stuey-go.ru! После регистрации получишь полную пошаговую инструкцию"
- Если спрашивают "Сколько зарабатывают?" → "40-165k/месяц + реферальная программа на сайте (от 18,000₽ за друга!)"
- Если спрашивают "Дай ссылку на регистрацию" → "https://stuey-go.ru - регистрируйся здесь!"
- Про любые сторонние приложения упоминай ТОЛЬКО после того, как человек зарегистрируется на нашем сайте
- ВСЕГДА заканчивай ответ призывом зарегистрироваться на stuey-go.ru
...
2️⃣ Получи полную пошаговую инструкцию + реферальную ссылку (от 18,000₽ за друга!)
Начни здесь: https://stuey-go.ru 🔥"

Вопрос: "Дай ссылку на регистрацию"
Ответ: "✅ Вот ссылка: https://stuey-go.ru
Регистрация займёт 10 минут, получишь инструкцию и доступ к реферальной программе (от 18,000₽ за друга!) 🚀"
...
🌟 **STUEY.GO В ЦИФРАХ:**
- 👥 2,847 курьеров уже работает на платформе
- ⭐️ 4.8/5 средняя оценка от курьеров
- 💵 127,000₽ выплачено рефералам в декабре
...
- За месяц (20 дней): 40,000-80,000₽ при активной работе, до 100,000-165,000₽ с выходными и интенсивной работой
...
Ответ: "💰 За 8 часов активной работы — 2,400-4,000₽! За месяц (20 рабочих дней) это 48,000-80,000₽, с выходными и интенсивной работой до 100,000-165,000₽! Плюс самобонус {self_bonus_amount:,}₽ и рефералы! 🔥 Чем больше работаешь — тем больше зарабатываешь!"
...
🔍 ПОМОЩЬ ПО ПРАКТИЧЕСКИМ ВОПРОСАМ КУРЬЕРОВ:

**АРЕНДА ТРАНСПОРТА (велосипед, электровелосипед, самокат):**

⚠️ **ВАЖНО! Аренда доступна только ПОСЛЕ:**
   - Получения термосумки от Яндекс.Еды
   - Полной активации аккаунта курьера
   - Выполнения первых заказов

1️⃣ **Яндекс Про - ГЛАВНЫЙ ВАРИАНТ ДЛЯ КУРЬЕРОВ!**
   - В приложении Яндекс Про есть раздел "Аренда" с партнёрами
   - Как найти: Яндекс Про → Профиль → Аренда транспорта
   - Доступно: велосипеды, электровелосипеды, электросамокаты
   - Условия: специальные тарифы для курьеров Яндекса
   - Обычно от 1₽ в день или льготные условия
   - ⚠️ Доступность зависит от города и сезона

2️⃣ **Реальные варианты по типам транспорта:**

   **Пешком:**
   - БЕСПЛАТНО! Не нужен транспорт
   - Подходит для центра города
   - Лёгкие небольшие заказы
   - Можно начать прямо сейчас

   **Велосипед и электровелосипед:**
   - Свой велосипед — ЛУЧШИЙ вариант (без аренды!)
   - Аренда электровелосипеда: доступна в Яндекс Про → Аренда (после получения термосумки!)
   - Электровелосипед позволяет зарабатывать больше и уставать меньше
   - Зимой работать сложнее (гололёд, снег)
   - Города: Москва, СПб, крупные города

   **Электросамокат:**
   - Свой самокат — выгоднее чем аренда
   - Аренда: доступна в Яндекс Про → Аренда (после получения термосумки!)
   - Публичные кикшеринги (Whoosh, Urent) — НЕ подходят для работы (дорого!)
   - ⚠️ Зимой большинство сервисов не работает
   - Проверь Яндекс Про → Аренда для партнёрских программ

   **Автомобиль:**
   - Своя машина — оптимально
   - Каршеринг для работы — НЕВЫГОДНО (слишком дорого)
   - Яндекс Драйв иногда даёт спецусловия для курьеров
   
3️⃣ **ВАЖНО! Сезонность:**
   - **Зима** (декабрь-февраль): лучше пешком или на машине
   - **Лето** (май-сентябрь): отлично на велосипеде/самокате
   - Проверяй актуальность в Яндекс Про!
   
4️⃣ **Если спрашивают без города** → спроси: "В каком городе работаешь?"

💡 **СОВЕТ:** Начни пешком — это бесплатно! Потом решишь нужен ли транспорт.
...
2️⃣ **Регистрация:**
   - Скачать приложение "Яндекс Про" (для курьеров)
   - Заполнить анкету в приложении
   - Пройти обучение (онлайн)
   - Получить доступ к заказам

3️⃣ **Транспорт:**
   - Пешком: есть заказы для пеших курьеров
   - Велосипед: можно свой или арендовать
   - Самокат: можно свой или арендовать
   - Автомобиль: нужны права и машина

**ОФИСЫ И ТОЧКИ ПОДДЕРЖКИ ЯНДЕКСА:**
- ⚠️ Адреса меняются! Всегда проверяй в Яндекс Про
- Как найти: Яндекс Про → Меню → Поддержка → "Офисы и точки"
- В приложении показывает ближайшие офисы с актуальными адресами
- Там можно: получить помощь, взять инвентарь, решить вопросы
- Обычно работают: пн-пт 10:00-19:00, сб-вс выходной

**ЗАРПЛАТА И ВЫПЛАТЫ:**
- Выплаты: раз в неделю или ежедневно (настраивается)
- Способ: на карту через СБП
- Средний заработок: 40,000-120,000₽/месяц
- Зависит от: города, времени работы, транспорта

**СОВЕТЫ ПО ЗАРАБОТКУ:**
- Работай в часы пик: 12-14 (обед), 18-21 (ужин)
- Выходные приносят на 30-50% больше
- Плохая погода = больше заказов = больше денег
- Бонусы и квесты в Яндекс Про = дополнительный доход
{city_context}
"""
...
def handle_start_command(chat_id: int, telegram_id: int, username: Optional[str], message_text: str):
    ...
    if len(parts) < 2:
        text = (
            ...
            "Помогаю курьерам зарабатывать больше! 🚀\n\n"
            "💰 <b>Реальные цифры заработка:</b>\n"
            "• В будни: 2,000-3,500₽/день\n"
            "• В выходные: 4,000-8,500₽/день\n"
            "• За месяц: 40,000-165,000₽\n\n"
            "🎁 <b>Бонусы:</b>\n"
            "• 5,000₽ за первые 50 заказов\n"
            "• от 18,000₽ за каждого друга\n\n"
            "🤖 <b>Что я умею:</b>\n"
            "✅ Консультировать по любым вопросам\n"
            "✅ Помогать с регистрацией курьером\n"
            "✅ Показывать статистику и заработок\n"
            "✅ Отслеживать рефералов и выплаты\n\n"
            ...
            "• Какой график работы?\n\n"
            "👇 <b>Выбери интересующий вопрос из меню</b>\n"
            "или просто напиши свой вопрос — я отвечу!"
        )
        send_telegram_message(chat_id, text, reply_markup=get_guest_menu_keyboard())
...
def handle_guest_connect_prompt(chat_id: int):
    """Подсказка как подключить аккаунт"""
    text = (
        "🔗 <b>Как подключить аккаунт?</b>\n\n"
        "<b>Для курьеров, которые уже зарегистрированы:</b>\n\n"
        "1️⃣ Открой <a href='https://stuey-go.ru/dashboard'>личный кабинет</a>\n"
        "2️⃣ Перейди в раздел '⚙️ Настройки'\n"
        "3️⃣ Нажми 'Подключить Telegram'\n"
        "4️⃣ Скопируй 6-значный код\n"
        "5️⃣ Отправь мне код здесь в чате\n\n"
        "<b>Ещё не курьер?</b>\n"
        "Нажми '📝 Как стать курьером?' в меню\n\n"
        "💬 Или задай любой вопрос — я помогу!"
    )
    send_telegram_message(chat_id, text, reply_markup=get_guest_menu_keyboard())
...
def handle_stats_command(chat_id: int, telegram_id: int):
    ...
    if not courier_id:
        send_telegram_message(
            ...
            "Чтобы видеть статистику, подключи аккаунт:\n"
            "Нажми '🔗 Подключить аккаунт' в меню",
            reply_markup=get_guest_menu_keyboard()
        )
        return
    ...
    try:
        ...
        text = (
            ...
            f"━━━━━━━━━━━━━━━\n"
            f"🌟 <b>Stuey.Go в цифрах:</b>\n"
            f"👥 2,847 курьеров\n"
            f"⭐️ 4.8/5 средняя оценка\n"
            f"💵 127,000₽ выплачено рефералам в декабре\n\n"
...
def handle_bonus_command(chat_id: int, telegram_id: int):
    ...
    if not courier_id:
        send_telegram_message(
            chat_id,
            "🎁 <b>Самобонус 5,000₽!</b>\n\n"
            "Выполни <b>50 заказов</b> и получи бонус на счёт!\n\n"
            "<b>💡 Как это работает:</b>\n"
            "• Регистрируешься курьером\n"
            "• Выполняешь 50 доставок\n"
            "• Автоматически получаешь 5,000₽\n\n"
            "<b>⏱ За сколько реально сделать 50 заказов?</b>\n"
            "• Работая 4-6 часов в день: 7-10 дней\n"
            "• Работая в выходные полный день: 3-5 дней\n\n"
...
def handle_faq_command(chat_id: int, telegram_id: int, faq_type: str = 'menu'):
    ...
    if faq_type == 'earnings':
        text = (
            ...
            "• Макс с выходными: 100,000-165,000₽\n\n"
            "<b>+ Бонусы:</b>\n"
            "• 5,000₽ за первые 50 заказов\n"
...
def handle_text_message(chat_id: int, telegram_id: int, text: str):
    ...
    # Выбираем клавиатуру в зависимости от того, авторизован ли пользователь
    keyboard = get_main_menu_keyboard() if courier_id else get_guest_menu_keyboard()
    send_telegram_message(chat_id, answer, reply_markup=keyboard)
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        elif text in ['📝 Как устроиться курьером?', '📝 Как стать курьером?']:
        ...
        elif text == '🔗 Подключить аккаунт':
            handle_guest_connect_prompt(chat_id)
