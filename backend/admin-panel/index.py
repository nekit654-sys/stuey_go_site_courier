...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if method == 'GET' and action == 'get_all_couriers':
            ...
            try:
                # Получение всех курьеров из таблицы users (базовый запрос без JOIN)
                cursor.execute("""
                    SELECT 
                ...
                for row in rows:
                    ...
                    # Получаем данные пригласившего (если есть)
                    inviter_name = None
                    inviter_code = None
                    if row[10]:  # invited_by_user_id
                        try:
                            cursor.execute("""
                    ...
                    couriers.append({
                        ...
                        'total_orders': int(row[14]) if row[14] else 0,
                        'self_bonus_amount': 0,  # Будет добавлено позже
                        'referral_income': 0,  # Будет добавлено позже
                    })
